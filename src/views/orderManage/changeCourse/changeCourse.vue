  ...
  <div class="changeCourse">
    <pane-head :nav="nav" :borderBottom="true" @pane-head-func="headFunc">
      <div slot="tool" class="tool-term">
        <el-button v-if="nav[navIdx] === '我的申请'" @click="apply" type="primary">申请</el-button>
      </div>
    </pane-head>
    <div class="pane">
      ...
      <div class="table-data">
        <el-table :data="tableData" v-loading='loading' stripe width="100%">
          <el-table-column prop="order" fixed width="160px" label="订单号"></el-table-column>
...
import { changeCourse } from '@/api/modules/OrderManage'
export default {
  name: 'changeCourse',
  components: {
    toolBtn
  ...
  methods: {
    ...
    apply () {
      this.$router.push({ name: 'changeCourseAdd' })
    },
    toolFunc (data) {
      if (data.btn === '修改') {
        this.$router.push({ name: 'changeCourseEdit' })
      } else if (data.btn === '详情') {
        this.$router.push({ name: 'changeCourseDetail' })
      }
    }
  },
  mounted () {
