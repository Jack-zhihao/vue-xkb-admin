    ...
    <div class="pane">
      ...
      <div class="table-data">
        <el-table :data="tableData"  v-loading="loading" stripe width="100%">
          ...
          <el-table-column prop="changeCourseType" label="换课类型"></el-table-column>
          <el-table-column prop="applyDisparityMoney" align="center" label="申请差价"></el-table-column>
          ...
          <el-table-column fixed="right" align="right" width="150" label="操作">
            <template slot-scope="scope">
              <tool-btn v-if="scope.row.examineStatus === '不通过'"  :btns="['删除','修改','详情']"></tool-btn>
              <tool-btn v-if="scope.row.examineStatus === '已通过'"  :btns="['详情']"></tool-btn>
              <tool-btn v-if="scope.row.examineStatus === '审核中'"  :btns="['取消申请','详情']"></tool-btn>
              <tool-btn v-if="scope.row.examineStatus === '待审核'"  :btns="['详情']"></tool-btn>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
...
import { zeroChangeCourse } from '@/api/modules/OrderManage'
export default {
  name: 'changeCourse',
  components: {
    toolBtn
